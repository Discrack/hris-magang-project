...
<head>
    ...
    <title>Edit Acara Kalender - Admin</title>
...
<body class="bg-gray-100 font-sans">
    <nav class="bg-blue-600 p-4 text-white flex justify-between items-center shadow-md">
        <div class="flex items-center">
            ...
            <h1 class="text-2xl font-bold">Edit Acara Kalender</h1>
        </div>
        <div class="flex items-center">
    ...
    <div class="container mx-auto p-6">
        <div class="bg-white p-8 rounded-lg shadow-md">
            <h2 class="text-3xl font-bold text-gray-800 mb-6 text-center">Form Edit Acara Kalender</h2>

            <form action="{{ route('admin.calendar.update', $event->event_id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <textarea id="description" name="description" rows="4"
                        class="shadow-sm appearance-none border rounded-lg w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('description', $event->description) }}</textarea>
                    @error('description')
                        <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                    @enderror
                </div>
