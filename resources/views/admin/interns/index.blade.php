...
<head>
    ...
    <title>Manajemen Peserta Magang - Admin</title>
...
<body class="bg-gray-100 font-sans">
    <nav class="bg-blue-600 p-4 text-white flex justify-between items-center shadow-md">
        <div class="flex items-center">
            ...
            <h1 class="text-2xl font-bold">Manajemen Peserta Magang</h1>
        </div>
        <div class="flex items-center">
    ...
    <div class="container mx-auto p-6">
        ...
        <div class="bg-white p-8 rounded-lg shadow-md mb-6">
            <h2 class="text-3xl font-bold text-gray-800 mb-6 text-center">Daftar Peserta Magang</h2>
            ...
            @if ($interns->isEmpty())
                <p class="text-center text-gray-600">Tidak ada peserta magang yang ditemukan.</p>
            @else
                        ...
                        <tbody class="text-gray-600 text-sm font-light">
                            @foreach ($interns as $intern)
                                <tr class="border-b border-gray-200 hover:bg-gray-100">
                                    ...
                                    <td class="py-3 px-6 text-left">{{ $intern->user->username ?? '-' }}</td>
                                    ...
                                    <td class="py-3 px-6 text-center">
                                        <div class="flex item-center justify-center space-x-2">
                                            <a href="{{ route('admin.interns.edit', $intern->intern_id) }}"
                                                class="bg-yellow-500 hover:bg-yellow-600 text-white font-bold py-1 px-3 rounded-lg text-xs transition duration-300">
                                                Edit
                                            </a>
                                            <form action="{{ route('admin.interns.destroy', $intern->intern_id) }}"
                                                method="POST"
                                                onsubmit="return confirm('Apakah Anda yakin ingin menghapus peserta magang ini? Semua data terkait (user, presensi, gaji, penilaian) akan dihapus.');">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit"
