...
<head>
    ...
    <title>Kalender Program Magang</title>
...
<body class="bg-gray-100 font-sans">
    <nav class="bg-blue-600 p-4 text-white flex justify-between items-center shadow-md">
        <div class="flex items-center">
            ...
            @endif
            <h1 class="text-2xl font-bold">Kalender Program Magang</h1>
        </div>
        <div class="flex items-center">
            @if(Auth::check())
    ...
    <div class="container mx-auto p-6">
        <div class="bg-white p-8 rounded-lg shadow-md">
            <h2 class="text-3xl font-bold text-gray-800 mb-6 text-center">Jadwal & Acara Penting</h2>

            @if ($events->isEmpty())
                <p class="text-center text-gray-600">Belum ada acara kalender yang dipublikasikan.</p>
            @else
                <div class="space-y-6">
                    @foreach ($events as $event)
                        <div class="bg-gray-50 p-6 rounded-lg border border-gray-200 shadow-sm">
                            ...
                            <p class="text-sm text-gray-500 mb-2">
                                Tanggal: {{ \Carbon\Carbon::parse($event->start_date)->format('d F Y') }}
                                @if ($event->end_date && $event->start_date != $event->end_date)
                                    - {{ \Carbon\Carbon::parse($event->end_date)->format('d F Y') }}
                                @endif
                            </p>
                            <p class="text-gray-700 leading-relaxed">{{ $event->description ?? 'Tidak ada deskripsi.' }}</p>
                        </div>
                    @endforeach
                </div>
            @endif
